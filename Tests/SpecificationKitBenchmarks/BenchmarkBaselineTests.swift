...
final class BenchmarkBaselineTests: XCTestCase {
    ...
    func testValidatorDetectsRegressionWhenMetricExceedsBaseline() {
        ...
        guard let regression = regressions.first else {
            XCTFail("Expected regression payload to include details")
            return
        }

        XCTAssertEqual(regression.metricName, "Specification Evaluation")
        XCTAssertEqual(regression.observedValue, 0.005, accuracy: 0.0001)
        XCTAssertEqual(regression.threshold, 0.001, accuracy: 0.0001)
    }
